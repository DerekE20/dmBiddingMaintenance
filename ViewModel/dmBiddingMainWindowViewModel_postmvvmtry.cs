...
namespace xBiddingMaintenance_1.ViewModel
{
     class dmBiddingMainWindowViewModel : INotifyPropertyChanged
    {
        ...
        public IList<stage> GetStages()
        {
            ...
            IList<stage> stages = (from stage in _ctx.stages select stage).ToList();
            return stages;
        }
        ...
        */
        ...
        /*private void bidstagesViewSource_Filter(object sender, FilterEventArgs e)
        {
            ...
            {
                // Filter logic here:
                string s = SelectedMethod.MethodDescription;
                string s1 = SelectedStage.StageDescription;
                int i = SelectedMethod.MethodId;
                int j = SelectedStage.StageId;
                if ((_bid.MethodRef == i) & (_bid.StageRef == j))
                {
                    e.Accepted = true;
                }
                else
       ...
       public method SelectedMethod
       {
        ...
        /*private void FillBid()
        {
            ...
            var q = (from bid in ctx.bids
                     ...
                     where bid.MethodRef== Method.MethodId
                     select book).ToList();
